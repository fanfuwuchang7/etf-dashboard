...
def main():
    ...
    if live:
        for code, name in ETFS:
            ...
            if rows:
                f = compute(rows)
                if f is None:        # K线不足(无效代码/抓取异常), 跳过该标的
                    skipped.append(code)
                    continue
                f["code"] = code
                f["name"] = name
    ...
    if skipped:
        print("  [提示] 跳过 %d 只K线不足/无效代码的ETF: %s" % (len(skipped), ", ".join(skipped)))
    ...
    print("[OK] 完成 (%s) | 大盘:%s 总仓%d%% | 推荐:%s | 风控通过:%d/%d | 回测信号样本:%d" % (
...
def render_backtest(records, bt, now_str, live):
    ...
    src = "新浪财经(实时)" if live else "新浪财经(离线快照)"
    return BACKTEST_TPL % (len(rows), now_str, src, body)
